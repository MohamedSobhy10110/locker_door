...
#define ARRAY_INDEX       5                            //size of pass code array
...
uint8 character;	                                   //Auxiliary variable
uint8 buffer[ARRAY_INDEX];                             //buffer to store entered pass code
uint8 buffer_received[ARRAY_INDEX];                    //buffer to store received pass code from another micro
uint8 buffer_newPass[ARRAY_INDEX];                     //buffer to store transmitted pass code to another micro
...
void receive_code(void)                         //function used to receive code from another micro
{
	...
	uint8 count = 0;
	for(count = 0;count < ARRAY_INDEX;count++)  //for loop to store received pass code
	{
...
{
	send_messg('r');                          //sending letter 'r' to make another micro receive new pass code
	uint8 count;
...
{
	if(chr != '+' && chr != '-')           //writing letter on LCD except '+','-' and store written pass code
	{
		if(character == '+')              //'+' used to write new code
		{
			if(buffer_index != ARRAY_INDEX)//if number of written less than 5 number which is the number of pass code
			{
				lcd_chr(&mylcd,chr);
		...
		{
			if(buffer_index != ARRAY_INDEX) //if letter written is '+' or '-' to make user Choosing mode
			{
			...
			if(buffer_index == ARRAY_INDEX)
			{
				...
				{
					gui_wrong_passcode();
					buffer_index = 0;       //to make user writing pass code again
				}
			}
		}
	}
	else
	{
		...
		{
			send_messg('o');             //sending letter 'o' to Open door
			lcd_clear(&mylcd);
			lcd_setCursor(&mylcd,0,0);
