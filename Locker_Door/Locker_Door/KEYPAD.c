...
{
	/*
	* Static Function The main Core Of Keypad
	* it's usage to know which key has been pressed
	* it called when it found one or more input pin low 
	* check which pin caused low by making one shifting column low
	*/
	...
	for(count_col = START_COL;count_col < COLUMS;count_col++)
	{
		...
		/* set all columns high
		  to stand out the pressed key i
		  need to make all the other out to be high and one only low*/
		
		pin_write(ptrkeypad->keypadCol[count_col], LOW_LEVEL); // clear a single column
...
void keypad_mainFun(keypadcnfg_t* ptrkeypad)
{
	...
	clear_pins(ptrkeypad->keypadCol[START_COL],ptrkeypad->keypadCol[END_COL]);// set Input pins
	for(count_row = START_ROW;count_row < ROWS;count_row++)
	{
